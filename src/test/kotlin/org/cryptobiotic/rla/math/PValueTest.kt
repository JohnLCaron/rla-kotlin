...
class PValueTest {
    ...
    @Test
    fun compareWithAudit() {
        var count = 0
        table3s.forEach {
            assertEquals(pValueApprox(it).toDouble(), pValue(it), .001, "$count: $it")
            count++
        }
    }
    ...
    fun problem() {
        val it = table3s[1]
        assertEquals(pValueApprox(it).toDouble(), pValue(it), .001)
    }
}

fun pValueApprox(row: T3row) = Audit.pValueApproximation(row.n, row.mu.toBigDecimal(), row.gamma.toBigDecimal(), 0, 0, row.n1, row.n2)
